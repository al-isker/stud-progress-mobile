...
import { useUpdateExpoPushTokenMutation } from '../../api/use-update-expo-push-token-mutation';

export const useUpdateExpoPushToken = () => {
	const updateExpoPushTokenMutation = useUpdateExpoPushTokenMutation();

	const handleSuccess = () => {
		AsyncStorage.removeItem(SHOULD_SENT_EXPO_PUSH_TOKEN_STORAGE_KEY);
	};

	const handleError = () => {
		AsyncJSONStorage.setItem(SHOULD_SENT_EXPO_PUSH_TOKEN_STORAGE_KEY, true);
	};
